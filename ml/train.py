...
def main():
    ...
    print(f"Ham dataset: {len(df)} ornek")
    print(df["attack_type"].value_counts().to_string())
    print()

    # OBFUSCATED dominant class'i 50 ornekle sinirla
    obf_mask = df["attack_type"] == "OBFUSCATED"
    df_obf   = df[obf_mask].sample(n=min(50, int(obf_mask.sum())), random_state=42)
    df       = pd.concat([df[~obf_mask], df_obf]).reset_index(drop=True)
    print(f"Dengelenmis dataset: {len(df)} ornek")
    print(df["attack_type"].value_counts().to_string())
    print()
    ...
    if HAS_SMOTE:
        ...
        if counts.min() < 6:
            ...
            for cls in counts[counts < 6].index:
                idxs = [i for i, v in enumerate(y_raw) if v == cls]
                while y_list.count(cls) < 6:
        ...
        if k >= 1:
            ...
            print(f"SMOTE: {len(X_raw)} -> {len(X)} ornek")
            print(pd.Series(y).map(dict(enumerate(le.classes_))).value_counts().to_string())
            print()
    else:
        print("UYARI: imbalanced-learn yuklu degil (pip install imbalanced-learn)\n")
    ...
    base_model = VotingClassifier([("rf", rf), ("gb", gb)], voting="soft")

    min_class_count = int(pd.Series(y).value_counts().min())
    n_splits = min(5, min_class_count)
    if n_splits >= 2:
        cv     = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)
        scores = cross_val_score(base_model, X, y, cv=cv, scoring="f1_weighted")
        print(f"Cross-val F1 (weighted): {scores.mean():.3f} +/- {scores.std():.3f}\n")
    else:
        print("Cross-validation skipped\n")

    model = CalibratedClassifierCV(base_model, cv=min(3, n_splits) if n_splits >= 2 else "prefit", method="isotonic")
    if n_splits < 2:
        base_model.fit(X, y)
    model.fit(X, y)
